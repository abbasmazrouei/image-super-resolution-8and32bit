...
img = cv.imread('D:/New folder/FinalCargo-1398-11-28 - for simulation - for bandar test/FinalCargo/bin/x64/Debug/Old DB/1000004569/doctor req/1.tiff')
#img3 = cv.imread('E:/flashhhhhhhhhhhhhhhhh/15.01.00/1Color.tiff')

#height, width = img2.shape[:2]
#height2, width2 = img3.shape[:2]

img1 = cv.resize(img, None, fx = 1, fy = 1, interpolation = cv.INTER_CUBIC)
...
sr_img2 = rdn.predict(img1, by_patch_of_size=200)
sr_img2 = cv.resize(sr_img2, None, fx = 1/4, fy = 1/4, interpolation = cv.INTER_CUBIC)
cv.imwrite('GANcubic1.tiff',sr_img2)
